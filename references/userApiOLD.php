<?php
    ...
    function allEvents(){
        ...
        foreach($reverseEvents as $event){
            ...
            //To validate if the user is already a participant or whatnot
            $isPart = false;
            ...
            echo '
                <div class="all-events">
                    ...
                    <h3 style="margin: 0;">◦ Category: ' .
                         $event['eventType'] . '<br>◦ Date: ' . $event['eventDate'] . '<br>◦ Time: ' . $event['eventTime'].'</h3>
                         <br>
                         <hr>
                    <p>◦ Administrator: '.$adName.'</p>
    ...
    //This function is to request to become an organizer
    function orgRequest($eventId, $uid){
        ...
        $new_notif = [
            ...
            "body" => " is requesting to become an organizer of the event."
        ];
            ...
            <div class="events-inner-container">
                ...
                <div class="details-container">
                    <div class="event-details">
                        <h2>Category: <?= $event['eventType'] ?></h2>
                        <h4>Date: <?= $event['eventDate'] ?> <br>Time: <?= $event['eventTime'] ?></h4>
                        <p>Administrator: <?= $adminDetail['name'] ?></p>
                        <p>Number of Participants: <?= $noParts ?></p>
                    </div>
...
<?php
    ...
    function acceptRequest($requestorId, $eventId){
        ...
        $new_notif = [
            ...
            "body" => "Your request to join ". $eventName ." event has been accepted. Get ready to break a leg! "
        ];
    ...
    function removeNotification($userId, $eventId) {
        $notifJSON = "data/notif.json";
         $stored_notif = json_decode(file_get_contents($notifJSON), true);

         foreach ($stored_notif as $key => $notif) {
             if ($notif['uid'] == $userId && $notif['eventId'] == $eventId) {
